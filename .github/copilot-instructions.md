...
**Soc Ops** is an interactive Social Bingo game for in-person mixers, built with **Python (FastAPI)** + **Jinja2 templates** + **HTMX** for real-time interactivity. Players find people matching bingo questions to mark squares and get 5 in a row. The app runs on the server with state retained via signed cookies.
...
- Cookie is sent with each HTMX request and updated on the response
...
- Responses include `HX-Trigger` headers to manage modal state
...
See [CSS Utilities](instructions/css-utilities.instructions.md) for available utility classes and patterns. The project uses a custom Tailwind-like system in `app/static/css/app.css` with no external CSS frameworks.

## Frontend Design

See [Frontend Design Skill](instructions/frontend-design.instructions.md) for creative component guidelines. When designing UI elements, create distinctive, cohesive aesthetics that avoid generic AI patterns.
